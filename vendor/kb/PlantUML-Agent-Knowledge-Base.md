...
### 12.7 系统功能结构图 / 系统功能架构图（WBS 图 · 国内高校推荐语法）

> **专业命名**：教学中常把「按子系统/模块层次展开的功能划分示意」称为 **「系统功能结构图」**或**「系统功能架构图」**；在软件工程与国际标准中，这类**层次化工作分解**图型的规范名称为 **「WBS 图」**（Work Breakdown Structure，**工作分解结构**）。  
> 与 **程序流程图（A 类 `@startuml activity`）**不同：WBS 描述的是**功能/模块的隶属与分解关系**，不是执行步骤或判定的先后次序。

**何时使用**：用户要求画功能结构、模块树、子系统划分、平台功能一览、层次架构（指功能分解而非部署/组件）、“像组织结构图那样从上到下拆功能”等 —— 只要语义是 **分解结构** 而非 **业务流程执行**，优先用 WBS。

**语法框架**（必须用成对关键字，**不要**写 `@startuml activity`）：

```plantuml
@startwbs
title 系统功能结构图（WBS）

' —— 纯白底、黑线、禁灰底（打印/作业常见要求）——
...
' 若环境支持 style，可强化节点白底（与版本有关，可二选一保留）
<style>
wbsDiagram {
  LineColor #000000
}
...
**硬约束**：
1. 根节点 `*` 通常为系统/产品总名；`**` 大块业务域或子系统；`***` 及以下为功能点（层次与作业要求一致即可）。  
2. **禁止**在 WBS 图里使用 `:开始;` / `start` / `stop` 等活动图节点（那是 A 类流程图专规）。  
3. 输出时若与「流程图」「活动图」混问，**拆成两幅图**或在回答中先说明本幅为 **WBS（功能分解）**。  
4. 方框背景须为 **#FFFFFF 纯白**；不要使用默认灰底节点外观；已用 `skinparam shadowing false` 与 `backgroundColor #FFFFFF` 作为基线。
