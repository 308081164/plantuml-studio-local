...
本地 **Java + 官方 PlantUML JAR（PicoWeb）** 渲染，可选 **DeepSeek** 生成 PlantUML 源码并走「生成 → 本地校验 → 自动修正」闭环。**2.0.0+** 系列为 **M3 / 开发计划第 11 节 Beta**：支持选择**项目目录**、本机生成目录摘要并结合自然语言一键制图。
...
- DeepSeek 与编排参数：`%AppData%\plantuml-studio-local\studio-agent-config.json`（含可选 `lastProjectRoot`）
